...
system which would support test beams, electronics developments,
cosmic ray tests, and the like.
...
features of this DAQ system. \emph{Each} instruction is a seperate command
...
We have so far used the ``long'' versions of each rdaq command, such as
...
Here are the aliases which you get:
...
\item{device\_file} this device absorbs the contents of another
  ...
  expected to be larger than 128KB, you need to say how big it is. RCDAQ
  needs to calculate the worst-case maximum event size per event type,
  although virtually all events will be smaller due to
  ...
  image to the data stream. If the file does not exist, no packet is generated.

\item{device\_file\_delete} \label{delete}like device\_file, except
  that this device also deletes the file after it has been read. {\bf
    Use this device with caution.} This device was added to allow you
  to \emph{occasionally} add some information to the data
  ...
  job, could generate the file with the temperature information. It will
  be added as a packet in the next event and subsquently deleted, so
  it would be added to one event only. In this way you could track the
...
\item{device\_filenumbers\_delete} like device\_filenumbers, except
...
\item{device\_command} this device does not add any data but executes
  ...
  defines a number of environmental variables to the process execting
  ...
  \item[DAQ\_FILENAME] The current file name. If logging is nor
...
filenames.  There is no practical limit to the number of such
...
\item[daq\_set\_maxbuffersize n\_KB] Adjust the size of buffers to n
  ...
  can largely be ignored. RCDAQ knows up front how large an event an
  become, worst case (if zero-suppression yields no savings etc). The
  DAQ is filling a memory buffer which holds many events; this is
  largely a storage-technology concept and generally invisible to
  you. Since the event gets assembled directly in the memory buffer,
  and the actual length is known only once the assembly is complete,
  RCDAQ needs to check that a maximum-sized event will still fit into
  the buffer. If not, it will start a new buffer, scheduling the other
  one to be written out. This command was available mostly for the
  benfit of the online monitoring, since the monitoring receives one
  buffer at a time. If you have a low data rate and small events, it
  can take several minutes (or even hours) for one buffer to be filled
  and the monitoring to receive data. This was the reason why one
  could reduce the size, to feed the online monitoring a more steady
  data stream. This was very sub-optimal, because the data taking
  efficiency is lower with smaller buffers, so one (small) size does
  not fit all. This brings us to the next command:
...
\item[daq\_set\_adaptivebuffering seconds] This is a much better
  ...
  rate-dependend adjustments. Unless there there is a specific reason,
...
I will expand on this chapter as time progesses.
...
ony 100Hz (remains to be seen). Then we would add, say, 150 microseconds
deadtime:
...
In this setup we use with a detector that gets positioned with a step
...
We use the \verb|streamer| utility to capture a webcam frame, and then copy
the resulting jpeg file to the packet 915:
...
rcdaq_client create_device device_command 9 0 "streamer -o pic.jpeg"
rcdaq_client create_device device_file 9 915 pic.jpeg 50000
# .. lines deleted...
\end{verbatim}

At each begin of a run, the command \verb|streamer -o pic.jpeg| is executed,
making a new file with a picture of the current detector position. 
...
snapshots to the begin-run event. We also have 4 different run types defined.
...
command. If that fails, we need to start the server. At this point,
we also define the Elog wherabouts. 
...
\begin{verbatim} 
$ dlist -O -i -t 9 beam_0000000451-0000.evt
...
\begin{verbatim} 
$ ddump  -O  -t 9 -p 940  /media/psf/Home/srs/GEM/beam/beam_0000000472-0000.evt
-0000020
$
\end{verbatim} 
...
\begin{verbatim} 
$ ddump  -O  -t 9 -p 940  /media/psf/Home/srs/GEM/beam/beam_0000000474-0000.evt
+0002380
$ 
\end{verbatim} 
...
\begin{verbatim} 
$ ddump  -O  -t 9 -p 941 beam_0000000472-0000.evt > 472.jpg
$ ddump  -O  -t 9 -p 942 beam_0000000472-0000.evt > 472_overhead.jpg
$ ddump  -O  -t 9 -p 941 beam_0000000474-0000.evt > 474.jpg
$ ddump  -O  -t 9 -p 942 beam_0000000474-0000.evt > 474_overhead.jpg
\end{verbatim} 
...
an ``install'' directory, and buld in the standard way. In this
...
have built the ``srs'' plugin here, as well as the
...
The installation of the srs plugin follows the same recipe:

\begin{verbatim} 
$ cd ../srs
$ pwd
/home/purschke/softwarerepo/build/srs
$ ../../srs/autogen.sh  --prefix=/home/purschke/softwarerepo/install
$ make install
\end{verbatim}
...
The CAEN V1742 module, shown in Fig.~\ref{caen1742v}, is one of the more complex modules implemented
in RCDAQ. At its core, it has 4 DRS4 chips inside, providing 32 primary input
channels, plus one ``side'' channel per chip. Two of those can be used to digitize the 
NIM input signal if so desired. 
...
speed of up to 5\,Gigasamples/s. Sample rates of 1, 2.5, and 5\,GS/s
are adjustable. This device has a large configuration space, with many
registers that allow a customized setup for many different purposes. 
...
phase space available that the V1742 has to offer. It comes with a
...
matter. Still, in this way one can set up any conveivable trigger
