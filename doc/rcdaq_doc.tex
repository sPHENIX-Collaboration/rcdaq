...
\item{device\_file\_delete} \label{delete}like device\_file, except
  ...
  be added as a packet in the next event and subsequently deleted, so
...
\item[daq\_webcontrol port\_number ] Start the web-based controls on
  the given port number (defaults to port 8080 if omitted). See
...
8080, but you can choose any port number that is available on your
system.
...
\end{verbatim}
starts the interface on the default port number 8080. You can then access the
