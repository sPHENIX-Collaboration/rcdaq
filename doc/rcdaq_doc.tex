...
\begin{itemize}
\item I brought the old file rollover feature from the AML days
  back. So far the second section field in fle names had not been used
  by RCDAQ. This is controlled by the \verb|daq_set_rollover| command
  and takes units of gigabytes (GB).
...
sequence number''. This number is used when files are rolled over after the
previous file reached its pre-set rollover limit.
...
\item[\bf device\_file] this device absorbs the contents of another
  ...
  expected to be larger than 16KB, you need to say how big it
...
\begin{verbatim}
   ...
   daq_set_rolloverlimit n_GB           set file rollover limit at n_BG GigaByte
...
\item[daq\_set\_rollover n\_GB] This will make RCDAQ ``roll over'' the
  output files after the given number of GigaBytes have been reached.
  This increments the usual second number field in the output files, such as
\begin{verbatim}
  rcdaq-00000001-0000.evt
  rcdaq-00000001-0001.evt
  rcdaq-00000001-0002.evt
  ...
\end{verbatim}
...
Here is an example how to set up such a pseudo device in a data event:

\begin{verbatim}
rcdaq_client create_device device_rtclock 1 1010
\end{verbatim}
